/*
 * Driver for Pixcir Tango C32/C44/C48 I2C touchscreen controllers.
 *
 ...
 *
 * pixcir_i2c_ts.c V3.3.0A	Add Android early power management
...
static struct point_node_t point_slot[16];	//static struct point_node_t point_slot[MAX_FINGER_NUM*2];
...
//		posy1 = ((Rdbuf[5] << 8) | Rdbuf[4]);		//Y 值 0 度
		posy1 = Y_MAX - ((rdbuf[5] << 8) | rdbuf[4]);		//Y 值 180 度
		posx2 = ((rdbuf[7] << 8) | rdbuf[6]);
//		posy2 = ((Rdbuf[9] << 8) | Rdbuf[8]);		//Y 值 0 度
		posy2 = Y_MAX - ((rdbuf[9] << 8) | rdbuf[8]);		//Y 值 180 度
	

	if(touch) {
		input_report_key(tsdata->input, BTN_TOUCH, 1);
		input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR, 15);
		//for (i=0; i<16; i++){		//for (i=0; i<MAX_FINGER_NUM*2; i++) {
		...
		//		point_slot[i].active = 0;
				...
				if(touch == 2)
				{
					input_report_key(tsdata->input, ABS_MT_TRACKING_ID, 1);
					input_report_abs(tsdata->input, ABS_MT_POSITION_X,  posx1);
					input_report_abs(tsdata->input, ABS_MT_POSITION_Y,  posy1);
					input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR, 15);
					input_mt_sync(tsdata->input);
...
static long pixcir_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case BOOTLOADER:	//BOOTLOADER = 7
		printk("pixcir_ioctl(),cmd = BOOTLOADER\n");
	...
	case RESET_TP:		//RESET_TP = 9
		printk("pixcir_ioctl(),cmd = RESET_TP\n");
		pixcir_reset();
		break;
	...
	case BOOTLOADER_STU:	//BOOTLOADER_STU = 16
		printk("pixcir_ioctl(),cmd = BOOTLOADER_STU\n");
		client->addr = BOOTLOADER_ADDR;
