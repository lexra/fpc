...
unsigned int WaitForSingleObject(HANDLE hEvent, unsigned int dwMilliseconds)
{
	...
	pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED, &last_type); // 強迫 PTHREAD_CANCEL_DEFERRED cancellation type
	pthread_mutex_lock(&cm);
	pthread_cleanup_push(CleanupLock, (void *)&cm);
	pthread_setcanceltype(last_type, NULL);						// 還原原來的 cancellation type
	while((cs & mask) != mask)
	{
		pthread_testcancel();
