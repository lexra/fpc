...
static void gp_adc_en ( int en )
{
	...
	clk_enable(pclk);
	if (pclk == NULL) {
		printk("no peripheral clock\n");
		sys_apb_clk = 27000000;
	}
	else
	{
		sys_apb_clk = clk_get_rate(pclk);
		printk("[gp_adc_en] pclk = %d\n",sys_apb_clk);
		clk_put(pclk);
	}
	...
	if( en ) {
		...
		adc_init.pclk_rate = sys_apb_clk;
...
int gp_adc_read(int handle)
{
	...
	printk("[%s] enter channel=%d\n", __func__, client->channel);
...
static void gp_adc_reg_restore()
{
	printk("[%s] enter\n", __func__);
	regADC_t *pAdc = (regADC_t *)(LOGI_ADDR_ADC_REG);
