...
printk("ax88179_bind()\n", msg_enable);
	...
	*tmp = AX_CLK_SELECT_ACS | AX_CLK_SELECT_BCS;
	...
	ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_NODE_ID, ETH_ALEN, ETH_ALEN, dev->net->dev_addr, 0);
	memcpy(dev->net->perm_addr, dev->net->dev_addr, ETH_ALEN);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
		netdev_dbg(dev->net, "ax88179_bind(), MAC [%02x-%02x-%02x-%02x-%02x-%02x]\n",
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34)
		netdev_dbg(dev->net, "ax88179_bind(), Monitor mode = 0x%02x\n", *tmp);
#else
