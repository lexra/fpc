...
int __init gp_gpio_test_init(void)
{
	gpio5_13_handle = gp_gpio_request(MK_GPIO_INDEX(2, 0, 62, 19), NULL);
	if(!gpio5_13_handle)
	{
		printk("get gpio5_13_handle fail!");
		gpio5_13_handle = 0;
		return -1;
	}
	printk("INIT OK\n");
	gp_gpio_set_function(gpio5_13_handle, GPIO_FUNC_GPIO);
	gp_gpio_set_direction(gpio5_13_handle, GPIO_DIR_OUTPUT);
	gp_gpio_set_pullfunction(gpio5_13_handle, GPIO_PULL_HIGH);

	gp_gpio_unregister_isr(gpio5_13_handle);
	gp_gpio_enable_irq(gpio5_13_handle, GPIO_IRQ_DISABLE);

	gp_gpio_set_output(gpio5_13_handle, 0, 0);
	//gp_gpio_set_value(gpio5_13_handle, 0);
	...
	//gp_gpio_unregister_isr(gpio5_13_handle);
	//gp_gpio_enable_irq(gpio5_13_handle, GPIO_IRQ_DISABLE);
	//gp_gpio_set_output(gpio5_13_handle, 1, 1);
...
void __exit gp_gpio_test_exit(void)
{
	if (gpio5_13_handle)
		gp_gpio_release(gpio5_13_handle);
}

//subsys_initcall(gpl8300_i2c_init);
module_init(gp_gpio_test_init);
module_exit(gp_gpio_test_exit);
