...
static int JkInclineSet(int fd, unsigned int value)
{
	int i = 0;
	...
	len = pack_ui_frame(0X0090, 0, 4, (unsigned char *)&data, buffer);
	...
	printf(">>InclineSet (0X0090): [VALUE]=%u %.01f%%\n", value, (float)value / 10.00F);
	return len;
}
...
void PrintUsage(void)
{
	...
	printf("Usage: %s -H [host] -P [connect port] -V [0.1%%]\n", file);
	printf("Example: %s -H localhost -P 34000 -V 111\n", file), exit(1);
}
...
//////////////////////////////////////////////////////////////////////
		...
		if (cmd == 0X0090)
		{
			...
			printf("<<InclineRsp (0X0090): [VALUE]=%u %.01f%%\n", val, (float)freq / 10.00F);
			break;
		}
		printf(".");
	}
